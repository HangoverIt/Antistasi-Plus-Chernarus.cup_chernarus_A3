...

{
	...
	if ((_unitType == staticCrewTeamPlayer) or (_unitType == SDKUnarmed) or (_unitType == typePetrovsky) or (_unitType in arrayCivs)) exitWith {_leave = true}
} forEach _unitsX;
if (_leave) exitWith {["Garrison", "Static crewmen, prisoners, refugees, Petrovsky or unknown units cannot be added to any garrison."] call A3A_fnc_customHint;};
