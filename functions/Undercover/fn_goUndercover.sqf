...
if (player == leader group player) then
{
    ...
    if (petrovsky in (units player)) then {[petrovsky] spawn A3A_fnc_undercoverAI};
};
...
private _reason = "";
private _polStns = ["polArea_1", "polArea_2", "polArea_3", "polArea_4", "polArea_5", "polArea_6", "polArea_7"];
...
while {_reason == ""} do
{
    ...
    if !(isNull _veh) then
    {
        ...
        {
            if (!(isOnRoad position _veh) && {(count (_veh nearRoads 50) == 0) || player inArea ([_roadblocks, player] call BIS_fnc_nearestPosition)}) then
            {
            ...
            if (_base in _roadblocks && ((primaryWeapon player != "") || (secondaryWeapon player != "") || (handgunWeapon player != "") || (vest player != "") || (getNumber(configfile >> "CfgWeapons" >> headgear player >> "ItemInfo" >> "HitpointsProtectionInfo" >> "Head" >> "armor") > 2) || (hmd player != "") || (!(uniform player in (A3A_faction_civ getVariable "uniforms"))))) then {_weapsDetected = true};
            if ((_onBaseMarker || _onDetectionMarker) && (_baseSide != teamPlayer)) then
            {
                if !(_isInRoadblock) then
                {
                    private _weapsFactor = (count (weaponCargo _veh)) * 10;
                    
                    private _aggro = if (_baseSide == Occupants) then {((aggressionOccupants + (tierWar * 10)) * 0.5) + _weapsFactor} else {((aggressionInvaders + (tierWar * 10)) * 0.5) + _weapsFactor};
                    //Probability of being spotted. Unless we're in an any "military" type outpost - then we're always spotted.
                    if (_base in (airportsX + milbases + outposts + seaports) || _onDetectionMarker || random 100 < _aggro || _weapsDetected == true) then
                    { sleep 2;
    ...
    else
    {
        ...
        if (detectionAreas findIf {player inArea _x} != -1) exitWith
        {
            _reason = "Restricted";
        };
    	...
    	private _polStn = [_polStns, player] call BIS_fnc_nearestPosition;
    	private _onPolStn = (player inArea _polStn);
    	if (_onPolStn) exitWith { _reason = "inPolStn" }
...
switch (_reason) do
{
    ...
    case "Restricted":
    {
        ["Undercover", "You entered a restricted area!"] call A3A_fnc_customHint;
    };
    case "VCompromised":
    {
    ...
    case "inPolStn":
    {
        ["Undercover", "You entered a police station uninvited!"] call A3A_fnc_customHint;
    };
    case "distanceX":
    {
