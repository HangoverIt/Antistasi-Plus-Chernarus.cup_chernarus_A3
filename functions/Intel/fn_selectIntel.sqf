    ...
    switch (_intelContent) do
    {
        ...
        case (DISCOUNT):
        {
            if (!(isTraderQuestCompleted || (!(isNil "isTraderQuestAssigned") && {isTraderQuestAssigned}))) then {
                ...
                _text = format ["We found some information about undiscovered hidden smuggler routes and gave them to the Arms Dealer. In return, he paid us for the information and gave us a %1 percent discount for any weapon in his Arms Dealer store.", _discount * 100];
            };
        };
    };
};
    ...
    switch (_intelContent) do
    {
        ...
        case (CONVOY_ROUTE):
        {
            if (!("CONVOY" in A3A_activeTasks) && !bigAttackInProgress) then
			{
                ...
                private _site = [_potentials, petrovsky] call BIS_fnc_nearestPosition;
