...
if(_isTrap) exitWith
{
    ...
    {
        [petrovsky,"hint","The screen says:<br/><br/>Prepare to die!", "Search Intel"] remoteExec ["A3A_fnc_commsMP",_x];
...
while {_pointSum <= _neededPoints} do
{
    ...
    if({[_x] call A3A_fnc_canFight} count _playerList == 0) exitWith
    {
        _pointSum = 0;
        {
            [petrovsky,"hint","No one in range of the intel, reseting download!", "Search Intel"] remoteExec ["A3A_fnc_commsMP",_x]
        } forEach ([50,0,_intel,teamPlayer] call A3A_fnc_distanceUnits);
    };
    ...
    {
        {
            [petrovsky,"intelError", _errorText] remoteExec ["A3A_fnc_commsMP",_x]
        } forEach _playerList;
    }
    else
    {
        ...
        {
            [petrovsky,"hintS", format ["Download at %1%2",((round ((_pointSum/_neededPoints) * 10000))/ 100), "%"], "Search Intel"] remoteExec ["A3A_fnc_commsMP",_x]
        } forEach _playerList;
    };
};
...
if(_pointSum >= _neededPoints) then
{
    ...
    {
        [petrovsky,"hint","You managed to download the intel!", "Search Intel"] remoteExec ["A3A_fnc_commsMP",_x];
