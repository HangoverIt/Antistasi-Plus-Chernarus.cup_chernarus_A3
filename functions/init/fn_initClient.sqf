...
_positionX = if (side player isEqualTo teamPlayer) then {position petrovsky} else {getMarkerPos "respawn_west"};
...
if (A3A_hasACE) then
{
	...
	} forEach (_playerUnits + [typePetrovsky, staticCrewTeamPlayer, SDKUnarmed, SDKSniper, SDKATman, SDKMedic, SDKMG, SDKExp, SDKGL, SDKMil, SDKSL, SDKEng]);
};
...
// only add petrovsky actions if he's static
if (petrovsky == leader group petrovsky) then {
	group petrovsky setGroupId ["Petrovsky","GroupColor4"];
	[petrovsky,"remove"] call A3A_fnc_flagaction;		// in case we already created them in initserver
	[petrovsky,"mission"] call A3A_fnc_flagaction;
};
petrovsky setIdentity "friendlyX";
if (worldName == "Tanoa") then {petrovsky setName "Maru"} else {petrovsky setName "Petrovsky"};
