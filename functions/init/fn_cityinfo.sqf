...
while {visibleMap} do {
	...
	if (count positionTel > 0) then {
		...
		_nameFaction = if (sidesX getVariable [_siteX,sideUnknown] == Occupants) then {
				nameOccupants
				} else {
				nameInvaders
		};
		...
		if (_siteX in citiesX) then {
			...
			_cityFaction = if !(gameMode == 4) then { nameOccupants } else { nameInvaders };
			_numCiv = _dataX select 0;
			...
			_textX = format ["%1<br/><br/>Pop %2<br/>%6 Support: %3 %5<br/>%7 Support: %4 %5",[_siteX,false] call A3A_fnc_location,_numCiv,_prestigeOPFOR,_prestigeBLUFOR,"%",_cityFaction,nameTeamPlayer];
