...
//	waitUntil {!isNil"statsLoaded"};
	...
	if (membershipEnabled and (membersX isEqualTo [])) then {
		[petrovsky,"hint","Membership is enabled but members list is empty. Current players will be added to the member list", "Membership"] remoteExec ["A3A_fnc_commsMP"];
...
[petrovsky,"hint","Server load finished", "Server Information"] remoteExec ["A3A_fnc_commsMP", 0];
...
[] execVM "JB_Missions\polMission.sqf";
