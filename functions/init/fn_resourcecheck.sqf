...
while {true} do
{
...

	{
		...
		if ((_supportGov < _supportReb) and (sidesX getVariable [_city,sideUnknown] == _governmentCitySide)) then
		{
			...
			[] call A3A_fnc_tierCheck;
				...
				if (_city in ["city_StarySobor","city_Vybor","city_Berezino","city_Solnychniy","City_Severograd","city_Elektrozavodsk","city_Chernogorsk","city_Svetloyarsk","city_Zelenogorsk","city_Novodmitrovsk","City_NovayaPetrovka"]) then {
				[_city, _governmentCitySide] spawn A3A_fnc_polCityChange;
				};
    ...
    if(_rebelsCount > 0) then {
        private _incomePerPlayer = round(_totalSalary / _rebelsCount);
        {
            ...
            private _paycheckText = format [
                "<t size='0.6'>%1 got paid <t color='#00FF00'>%2%3</t> for fighting for Chernarus</t>",
                name _x,
                _incomePerPlayer, 
				currencySymbol
            ];

            [petrovsky, "income", _paycheckText] remoteExec ["A3A_fnc_commsMP", _x];
        } forEach _rebels;
	...
	_hrAdd = ceil (_hrAdd/5);
	_resAdd = ceil (_resAdd/2);
		
	if ((server getVariable "hr") > (tierWar * 20)) then {_hrAdd = 0};
	...
	[petrovsky, "taxRep", _textX] remoteExec ["A3A_fnc_commsMP", [teamPlayer, civilian]];
