...
//added by JB, headshots instakill, executing injured enemy is bad...
if (_damage >= 1 && {_hitPoint == "hithead"}) exitWith {
	_unit removeAllEventHandlers "HandleDamage";
	if (isplayer _injurer) then {
		if (lifeState _unit == "INCAPACITATED" && _injurer distance _unit < 5) then {
			[_unit,"Executing injured combatants is a war crime. Word is spreading of your callousness..."] remoteExec ["globalChat",0];
			if (side group _unit == Occupants) then {
				[0,-2,getPos _unit] remoteExec ["A3A_fnc_citySupportChange",2];
        		[side group _unit, 10, 30] remoteExec ["A3A_fnc_addAggression", 2];
        	};
        	if (side group _unit == Invaders) then {
        		[side group _unit, 10, 30] remoteExec ["A3A_fnc_addAggression", 2];	
			};
        };
	} else {};        
