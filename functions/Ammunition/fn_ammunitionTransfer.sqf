...
if (_destinationX == boxX) then {
	...
	if (_updated != "") then {
		...
		[petrovsky,"income",_updated] remoteExec ["A3A_fnc_commsMP",[teamPlayer,civilian]];
	};
} else {
	[petrovsky,"hint","Truck Loaded", "Cargo"] remoteExec ["A3A_fnc_commsMP",driver _destinationX];
};
