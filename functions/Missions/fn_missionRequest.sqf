...
if (isNil "_type" or leader group petrovsky != petrovsky) exitWith { A3A_missionRequestInProgress = nil };
if (_type in A3A_activeTasks) exitWith {
	if (!_silent) then {[petrovsky,"globalChat","I already gave you a mission of this type."] remoteExec ["A3A_fnc_commsMP",_requester]};
...
switch (_type) do {
	case "AS": {
		...
		if (count _possibleMarkers == 0) then {
			if (!_silent) then {
				[petrovsky,"globalChat","I have no assassination missions for you. Move our HQ closer to the enemy."] remoteExec ["A3A_fnc_commsMP",_requester];
				[petrovsky,"hint","Assassination Missions require cities, patrolled Jungles or Airports closer than 4Km from your HQ.", "Missions"] remoteExec ["A3A_fnc_commsMP",_requester];
			};
		} else {
	...
	case "CON": {
		...
		if (count _possibleMarkers == 0) then {
			if (!_silent) then {
				[petrovsky,"globalChat","I have no Conquest missions for you. Move our HQ closer to the enemy."] remoteExec ["A3A_fnc_commsMP",_requester];
				[petrovsky,"hint","Conquest Missions require roadblocks or outposts closer than 4Km from your HQ.", "Missions"] remoteExec ["A3A_fnc_commsMP",_requester];
			};
		} else {
			private _site = selectRandom _possibleMarkers;
	...
	case "DES": {
		...
		if (count _possibleMarkers == 0) then {
			if (!_silent) then {
				[petrovsky,"globalChat","I have no destroy missions for you. Move our HQ closer to the enemy."] remoteExec ["A3A_fnc_commsMP",_requester];
				[petrovsky,"hint","Destroy Missions require Airbases or Radio Towers closer than 4Km from your HQ.", "Missions"] remoteExec ["A3A_fnc_commsMP",_requester];
			};
		} else {
			private _site = selectRandom _possibleMarkers;
			switch (true) do {
	...
	case "LOG": {
		...
		if (count _possibleMarkers == 0) then {
			if (!_silent) then {
				[petrovsky,"globalChat","I have no logistics missions for you. Move our HQ closer to the enemy."] remoteExec ["A3A_fnc_commsMP",_requester];
				[petrovsky,"hint","Logistics Missions require Outposts, Seaports or Banks closer than 4Km from your HQ.", "Missions"] remoteExec ["A3A_fnc_commsMP",_requester];
			};
		} else {
			private _site = selectRandom _possibleMarkers;
			switch(true) do {
	...
	case "SUPP": {
		...
		if (count _possibleMarkers == 0) then {
			if (!_silent) then {
				[petrovsky,"globalChat","I have no support missions for you. Move our HQ closer to the enemy."] remoteExec ["A3A_fnc_commsMP",_requester];
				[petrovsky,"hint","Support Missions require Cities closer than 4Km from your HQ.", "Missions"] remoteExec ["A3A_fnc_commsMP",_requester];
			};
		} else {
	...
	case "RES": {
		...
		if (count _possibleMarkers == 0) then {
			if (!_silent) then {
				[petrovsky,"globalChat","I have no rescue missions for you. Move our HQ closer to the enemy."] remoteExec ["A3A_fnc_commsMP",_requester];
				[petrovsky,"hint","Rescue Missions require Cities or Airports closer than 4Km from your HQ.", "Missions"] remoteExec ["A3A_fnc_commsMP",_requester];
			};
		} else {
			private _site = selectRandom _possibleMarkers;
	...
	case "CONVOY": {
		if (bigAttackInProgress) exitWith {
			if (!_silent) then {
				[petrovsky,"globalChat","There is a big battle around, I don't think the enemy will send any convoy."] remoteExec ["A3A_fnc_commsMP",_requester];
				[petrovsky,"hint","Convoy Missions require a calmed status around the island, and now it is not the proper time.", "Missions"] remoteExec ["A3A_fnc_commsMP",_requester];
			};
		};
		...
		{
			if (!_silent) then {
				[petrovsky,"globalChat","I have no Convoy missions for you. Move our HQ closer to the enemy."] remoteExec ["A3A_fnc_commsMP",_requester];
				[petrovsky,"hint","Convoy Missions require nearby enemy facilities, with a road route to an idle base within 3km.", "Missions"] remoteExec ["A3A_fnc_commsMP",_requester];
			};
		} else {
...
if (count _possibleMarkers > 0) then {
	if (!_silent) then {[petrovsky,"globalChat","I have a mission for you!"] remoteExec ["A3A_fnc_commsMP",_requester]};
