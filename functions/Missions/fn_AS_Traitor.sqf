...
private _typeVeh = if (_side == Invaders) then {selectRandom vehCSATLightUnarmed} else {selectRandom vehNATOLightUnarmed};
_veh = _typeVeh createVehicle _posVeh;
...
if ({_traitor knowsAbout _x > 1.4} count ([500,0,_traitor,teamPlayer] call A3A_fnc_distanceUnits) > 0) then
	...
	{[_x] orderGetin true} forEach units _groupTraitor;
...
if (not alive _traitor || traitorIntel) then
{
	...
	{
		{[petrovsky,"hint","Someone found some intel on the traitors family, he will not cause any problems any more!"] remoteExec ["A3A_fnc_commsMP",_x]} forEach ([500,0,_traitor,teamPlayer] call A3A_fnc_distanceUnits);
