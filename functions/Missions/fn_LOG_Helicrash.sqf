...
//crash position could be too close to miller, lets find out it again
...
private _rebelTaskText = format [
    "A %1 helicopter carrying weapons and supplies has crashed near %2. Bring the helicopter's cargo to the HQ before enemy's rescue team. We must get it before %3.", 
    _sideName, 
    _destinationName,
    _displayTime
];
...
//find endsites
private _endSites = (airportsX + milbases + outposts + seaports) select {sidesX getVariable [_x,sideUnknown] == _sideX};
private _nearestEndSite = [_endSites, getPos _helicopter] call BIS_Fnc_nearestPosition;

private _airports = (airportsX) select {sidesX getVariable [_x,sideUnknown] == _sideX};
...
private _escortWP = _escortVehicleGroup addWaypoint [_crashPosition, 20];
...
if(_cargoVehicle distance _box < 50 && {alive _cargoVehicle} && {!isNull (driver _cargoVehicle)}) then {
    ...
    _cargoWp2 = _cargoVehicleGroup addWaypoint [(getMarkerPos _nearestEndSite), 1];
    ...
    _escortWp1 = _escortVehicleGroup addWaypoint [(getMarkerPos _nearestEndSite), 2];
    ...
    _searchHeliWp1 = _heliVehicleGroup addWaypoint [(getMarkerPos (selectRandom _airports)), 2];
    _searchHeliWp1 setWaypointType "GETOUT";
    if(_isEnemyKnowsAboutTeamplayer) then {
...
waitUntil {
	sleep 1;
	!alive _box ||
	_box distance (getMarkerPos _nearestEndSite) < 50 ||
	_box distance (getMarkerPos respawnTeamPlayer) < 50 ||
	((dateToNumber date > _dateLimitNum) && ({side _x == teamPlayer} count nearestObjects [getPos _box,["Man","Car","Tank"],100] == 0))
};

switch(true) do {
    case(_box distance (getMarkerPos _nearestEndSite) < 50 || ((dateToNumber date > _dateLimitNum) && ({side _x == teamPlayer} count nearestObjects [getPos _box,["Man","Car","Tank"],100] == 0))): {
