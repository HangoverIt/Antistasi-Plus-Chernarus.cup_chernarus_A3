...
if (_varName in _specialVarLoads) then {
	...
	if (_varName == 'posHQ') then {
		...
		petrovsky setPos _posHQ;
		"Synd_HQ" setMarkerPos _posHQ;
		if (chopForest) then {
			if (!isMultiplayer) then {{ _x hideObject true } foreach (nearestTerrainObjects [position petrovsky,["tree","bush"],70])} else {{ _x hideObjectGlobal true } foreach (nearestTerrainObjects [position petrovsky,["tree","bush"],70])};
		};
	...
	if (_varName == 'datesSinceLastBaseMineChecks') then {
		datesSinceLastBaseMineChecks = _varValue ;
	};
} else {
