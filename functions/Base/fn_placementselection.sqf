...
if (_newGame) then {
	...
	format ["%1 is Dead",name petrovsky] hintC format ["%1 has been killed. You lost part of your assets and need to select a new HQ position far from the enemies.",name petrovsky];
};
...
if (visiblemap) then {
	if (_newGame) then {
		...
		petrovsky setPos _positionClicked;
	} else {
		...
		[_positionClicked] remoteExec ["A3A_fnc_createPetrovsky", 2];
	};
