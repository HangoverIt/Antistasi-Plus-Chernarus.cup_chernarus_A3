...
private _groupPetrovsky = group petrovsky;
[petrovsky] join theBoss;
deleteGroup _groupPetrovsky;

petrovsky setBehaviour "AWARE";
petrovsky enableAI "MOVE";
...
if (count _garrison > 0) then
{
    ...
    else
    {
        ...
        {
            if ((side (group _x) == teamPlayer) && (!(_x getVariable ["spawner",false])) && (_x distance2D _posHQ < _size) && (_x != petrovsky)) then
            {
                if (!alive _x) then
                {
...
petrovsky addAction ["Build HQ here", A3A_fnc_buildHQ, nil, 0, false, true];
