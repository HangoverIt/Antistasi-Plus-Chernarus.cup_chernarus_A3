...
switch (_typeX) do {
	...
	case "unlock": {
		switch (tierWar) do {
			case (2): {
				_textX = format ["<br/><t size='0.6'>Rebel vehicles stock has been updated with new vehicles. Addtional troop types can be recruited.</t>"];
				if (player == theBoss) then {
					_textX = _textX + format ["<br/><t size='0.6'>MG squads and MG cars are available for recruitment. Roadblocks and MG Emplacements are available for building.</t>"];
				};
			};

			case (3): {
				_textX = format ["<br/><t size='0.6'>Addtional troop types can be recruited.</t>"];
				if (player == theBoss) then {
					_textX = _textX + format ["<br/><t size='0.6'>AT teams are available for recruitment.</t>"];
				};
			};
			...
			case (4): {
				...
				if (player == theBoss) then {
					_textX = _textX + format ["<br/><t size='0.6'>AA, AT Emplacements are available for building. AA, AT cars and support squads are available for recruitment.</t>"];
				};
			};
		...
		if(tierWar > 3 && {!isPowPaycheckAnnounced}) then {
			_textX = _textX + format ["<br/><br/><t size='0.6'>The resistance has been recognized by <t size='0.6' color='#FFA500'>IDAP</t>, a third party peace-keeping organization and an agreement has been made - <br/><t size='0.6' color='#FFA500'>IDAP</t> will pay <t size='0.6' color='#008000'>%1</t> for freeing <t size='0.6' color='#0033CC'>%2</t> soldiers.</t>", nameTeamPlayer, nameOccupants];
