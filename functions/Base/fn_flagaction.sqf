...
switch _typeX do
{
    ...
    case "mission":
    {
        petrovsky addAction ["Mission Request", {CreateDialog "missionMenu";},nil,0,false,true,"","(isPlayer _this) and (vehicle _this == _this) and (_this == _this getVariable ['owner',objNull]) and (_this call A3A_fnc_isMember) and (petrovsky == leader group petrovsky)",4];
        petrovsky addAction ["HQ Management", {
            closeDialog 0;
		    closeDialog 0;
        ...
        },nil,0,false,true,"","(isPlayer _this) and (_this == theBoss) and (vehicle _this == _this) and (petrovsky == leader group petrovsky)", 4];
        petrovsky addAction ["Move this asset", A3A_fnc_moveHQObject,nil,0,false,true,"","(_this == theBoss)"];
    };
    case "truckX":
    {
